...
with tab1:
    ...
    if csv:
        ...
        if target and target in df.columns:
            ...
            cleaned_df = pd.concat([X, pd.Series(y, name=target)], axis=1)
            ...
            csv_buffer = df_metrics.to_csv(index=False).encode('utf-8')
            st.download_button("📥 Download CSV Report (Metrics only)", csv_buffer, file_name="report_metrics.csv", mime="text/csv")
