...
st.title("🤖 Enhanced AutoML App with Deep Learning & Explainability")
...
if uploaded_file:
    ...
    if algo in ["Logistic Regression","Decision Tree","Random Forest","Naive Bayes","KNN","SVM",
        ...
        elif algo=="Label Propagation": model = LabelPropagation()
        elif algo=="Label Spreading": model = LabelSpreading()
        ...
        st.write(px.scatter(pd.DataFrame(X_scaled, columns=X.columns), x=X.columns[0], y=X.columns[1], color=clusters))
    elif algo=="DBSCAN":
        ...
        st.write("Agglomerative Clustering labels:", set(labels))
    elif algo=="PCA":
        pcs = PCA(n_components=2).fit_transform(X_scaled)
        st.write(px.scatter(x=pcs[:,0], y=pcs[:,1], title="PCA 2D"))
    elif algo=="t-SNE":
        ts = TSNE(n_components=2).fit_transform(X_scaled)
        st.write(px.scatter(x=ts[:,0], y=ts[:,1], title="t-SNE 2D"))

    if show_perf:
        st.subheader("📊 Performance")
        st.write("Accuracy:", accuracy_score(y_test, y_pred))
        st.write("Confusion Matrix"); st.dataframe(confusion_matrix(y_test, y_pred))
        ...
        if algo=="Random Forest":
            ...
            explainer = lime.lime_tabular.LimeTabularExplainer(
                X_train, feature_names=X.columns, class_names=np.unique(y).astype(str), discretize_continuous=True
            )
            i = np.random.randint(0, X_test.shape[0])
            exp = explainer.explain_instance(X_test[i], model.predict_proba, num_features=5)
            st.write(f"LIME Explanation for instance #{i}")
            st.components.v1.html(exp.as_html(), height=350)

    # 🌐 Deep Learning demos
    st.sidebar.subheader("🧠 Deep Learning Demos")
    dl = st.sidebar.selectbox("DL Demo", ["None", "CNN Toy", "RNN Toy"])
    if dl!="None":
        st.subheader(f"{dl} Demo")
        if dl=="CNN Toy":
            Xc = X_scaled[:, :10].reshape(-1, 2, 5, 1)
            yc = tf.keras.utils.to_categorical(y, num_classes=len(np.unique(y)))
            m = Sequential([Conv2D(8,(2,2),activation='relu',input_shape=(2,5,1)), Flatten(),
                            Dense(16, activation='relu'), Dense(yc.shape[1], activation='softmax')])
        else:
            Xt = X_scaled.reshape(-1, X_scaled.shape[1], 1)
            yt = tf.keras.utils.to_categorical(y)
            m = Sequential([LSTM(16, input_shape=(Xt.shape[1],1)), Dense(yt.shape[1], activation='softmax')])

        m.compile(loss='categorical_crossentropy', optimizer='adam', metrics=['accuracy'])
        h = m.fit(Xc if dl=="CNN Toy" else Xt, yc if dl=="CNN Toy" else yt,
                  epochs=5, validation_split=0.2, verbose=0)
        fig = px.line(x=list(range(len(h.history['loss']))), y=[h.history['loss'], h.history['val_loss']],
                      labels={'x':'Epoch','value':'Loss'}, title="Training Loss")
        st.plotly_chart(fig)
        st.write("Accuracy:", h.history['accuracy'][-1], "Val Accuracy:", h.history['val_accuracy'][-1])

st.sidebar.markdown("---")
st.sidebar.markdown("🚀 Includes: Full ML, DL demos, AutoEDA, Explainability (SHAP/LIME)")
